...
function scheduler:onTriggle()
	if self:isAlive() then 
		...
		if self.callback then 
			-- self.callback(self.target, self.otherArgs[1], self.otherArgs[2], self.otherArgs[3])
			local status, ret = pcall(self.callback, self.target, self.otherArgs[1], self.otherArgs[2], self.otherArgs[3])
			if not status then
				Log.e(TAG, "%s", ret)
			end
		end 
